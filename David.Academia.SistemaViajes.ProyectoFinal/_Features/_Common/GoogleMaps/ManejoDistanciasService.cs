...
public class ManejoDistanciasService
{
    ...
    public async Task<decimal> ObtenerDistanciaEntreSucursalColaborador(decimal? latitudOrigen, decimal? longitudOrigen, decimal? latitudDestino, decimal? longitudDestino)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(latitudOrigen), "La latitud debe estar entre -90 y 90.");
        }

        if (longitudOrigen < -180 || longitudOrigen > 180)
        {
            throw new ArgumentOutOfRangeException(nameof(longitudOrigen), "La longitud debe estar entre -180 y 180.");
        }
        if (latitudDestino < -90 || latitudDestino > 90)
        {
            throw new ArgumentOutOfRangeException(nameof(latitudDestino), "La latitud debe estar entre -90 y 90.");
        }

        if (longitudDestino < -180 || longitudDestino > 180)
        {
            throw new ArgumentOutOfRangeException(nameof(longitudDestino), "La longitud debe estar entre -180 y 180.");
        }
        string url = $"https://maps.googleapis.com/maps/api/distancematrix/json?" +
                     $"origins={latitudOrigen},{longitudOrigen}&destinations={latitudDestino},{longitudDestino}" +
                     $"&key={_googleMapsSettings.ApiKey}&units=metric";
    ...
    public async Task<Respuesta<decimal>> CalcularDistanciaTotalAjustadaAsync(List<ColaboradorConKmsDto> colaboradores, decimal umbralProximidadKm)
    {
        ...
        var colaboradoresProcesados = new List<int>();
    ...
    public async Task<Respuesta<bool>> ValidarDistanciaEntreColaboradoresExistentesEnViajeAsync(List<ColaboradorConKmsDto> colaboradoresAsignados, List<ColaboradorConKmsDto> nuevosColaboradores, decimal umbralMaximoKm)
    {
        ...
        foreach (var nuevoColaborador in nuevosColaboradores)
        {
            foreach (var colaboradorAsignado in colaboradoresAsignados)
            {
                ...
                decimal distanciaColaboradorMayor;
                if (nuevoColaborador.DistanciaKms > colaboradorAsignado.DistanciaKms)
                {
                    distanciaColaboradorMayor = nuevoColaborador.DistanciaKms;
                }
                else
                {
                    distanciaColaboradorMayor = colaboradorAsignado.DistanciaKms;
                }
                ...
                {
                    respuesta.Valido = false;
                    respuesta.Mensaje = $"El colaborador {nuevoColaborador.Nombre} esta fuera del rango de proximimidad con el colaborador {colaboradorAsignado.Nombre} por {distanciaEntreColaboradores} de diferencia, favor crear un nuevo viaje para estos colaboradores";
                    return respuesta;
                }
            }
        }
